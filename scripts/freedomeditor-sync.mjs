...
function extensionCli(config, release, args) {
	...
	], { cwd: release.path, env: { ...process.env, ELECTRON_RUN_AS_NODE: '1', VSCODE_DEV: '1', NODE_ENV: 'development' }, stdio: 'inherit', timeout: 15 * 60_000 });
}
...
function launch(config, args) {
	...
	launchArgs.push(...(args.length ? args : [sourceRoot]));
	const environment = { ...process.env, NODE_ENV: 'development', VSCODE_DEV: '1', VSCODE_CLI: '1' };
	delete environment.ELECTRON_RUN_AS_NODE;
